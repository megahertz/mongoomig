...
class Config {
  constructor(config = {}) {
    ...
    this.debug             = this.def(config, 'debug', config.d, false);
    ...
    this.require = Array.isArray(this.require) ? this.require : [this.require];
    this.require = this.require.filter(req => req && req.length > 0);
  }
